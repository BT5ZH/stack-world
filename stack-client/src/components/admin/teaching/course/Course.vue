  ...
  <a-row class="container">
    <a-row :span="20" style="margin: 10px 25px 20px 5px;">
      <a-tree-select style="width: 100%" :value="value" :dropdown-style="{ maxHeight: '400px', overflow: 'auto' }"
        ...
        @select="onSelect">
        <a-tree-select-node :key="item._id" :value="`${item._id}#`" :title="item._id" v-for="item in treeData">
          <a-tree-select-node :key="mName" :value="`${mName}:${item._id}`" :title="mName" v-for="mName in item.majors">
          </a-tree-select-node>
        </a-tree-select-node>
      </a-tree-select>
    </a-row>
    <a-row :span="20">
      <a-tabs :active-key="activeIndex" @change="callback">
        <a-tab-pane key="1" tab="统计信息" force-render>
          <course-dashboard class="class-dashboard" :TreeData="treeData" :courseProp="courseNumber"
            :majorProp="majorList" :collegeProp="collegeList" :pieProp="pieArr"></course-dashboard>
        </a-tab-pane>
      ...
      async getTreeData() {
        ...
        try {
          ...
          this.treeData.forEach((item) => {
            ...
            item.majors.forEach((name) => {
              this.majorList.push(name);
            })
          })
