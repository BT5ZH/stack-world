<template>
    <div class="card_rectangle" @click.stop='handleRes(item)'>
        <div class="card_rectangle--row">
            ...
            <a-tag v-if='item.task_type' :color="item.task_type=='preview'?'cyan':(item.task_type=='homework'?'purple':'red')">
                {{item.task_type=='preview'?'预习':(item.task_type=='homework'?'作业':'考试')}}
            </a-tag>
        ...
        methods: {
            handleRes(item){
                if(item.resType==1){
                    ...
                    this.isClick==0? this.download(item.attachment_url):(this.isClick==3?this.jmpExam(item):this.jmpHomework(item));
                }
            },
            jmpTest(item){
            ...
            jmpExam(item){
                this.$router.push({ path: this.$route.path + '/exam/'+item.id,query: { title: item.title,} });
            },
            download(url){
                window.open(url);
            }
