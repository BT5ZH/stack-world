  ...
  <div>
    <a-row class="course_menu">
      <a-col :span="6" :key="index" v-for="(item, index) in courseDetailMenu">
        <!-- 菜单栏 -->
        <button :class="isClick == index ? 'courseDetailMenu active' : 'courseDetailMenu'" @click="changeNav(index)">
          {{ item.title }}
        </button>
      </a-col>
    </a-row>
    <div class="course_content" v-if="isClick == 0">
      ...
      <!-- <leftSlider -->
      <resCard v-else v-for="item in resList" :key="item.hid" :item="item" :isClick="isClick" :courseId="courseId">
      </resCard>
      <!-- </leftSlider> -->
    </div>
    ...
    <div class="course_content" v-if="isClick == 2">
      ...
      <resCard v-else v-for="item in homeworkList" :key="item.hid" :item="item" :isClick="isClick" :courseId="courseId">
      </resCard>
    </div>

    <div class="course_content" v-if="isClick == 3">
      <a-empty v-if="examList.length == 0" />
      <resCard v-else v-for="item in examList" :key="item.id" :item="item" :isClick="isClick" :courseId="courseId">
      </resCard>
    </div>
  </div>
</template>
