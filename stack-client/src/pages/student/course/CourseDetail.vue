    ...
    <div>
        <a-row class="course_menu">
            <a-col :span="8" :key='index' v-for="(item,index) in courseDetailMenu">
                <button :class="isClick==index?('courseDetailMenu active'):('courseDetailMenu')"
                    @click='changeNav(index)'>{{item.title}}</button>
            </a-col>
        </a-row>
...
<script>
    import leftSlider from '@/components/student/LeftSlider.vue';
    import resCard from '@/components/student/cards/ResCard.vue';
