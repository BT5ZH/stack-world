<template>
  <div>
    ...
    <div class="course_content" v-if="isClick == 0">
      ...
      <leftSlider v-else v-for="item in resList" :key="item.id" :item="item" :isClick="isClick" :courseId="courseId">
      </leftSlider>
    </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      socket.createInstance(this, {public:(data) => {
      if(data.lessonId === this.courseId) {
        socket.sendEvent({roomId:"joinRoom"})
      }
    }})
    },
    computed: {
